...
public interface ShipmentRepository extends JpaRepository<Shipment, Long> {
    ...
    @Query(value="Select s.id from shipment s join profileshop.jhi_order o on s.reference = o.reference and s.shipment_status = 'ARRIVED' and s.shipment_type='CUSTOMER' where o.id = :id", nativeQuery=true)
    ...
    @Query(value="SELECT * FROM (select p.id AS id,p.purchase_id AS po,p.description AS description,p.shipping_instructions AS shippingInstructions,p.quantity AS quantity,p.price AS cost,p.product_id AS productId,sum(ifnull((sr.accepted + sr.rejected),0)) AS preallocated,(select o.sku from (profileshop.order_item o join profileshop.purchase_item_order_item pioi on((o.id = pioi.order_item_id))) where (pioi.purchase_item_id = p.id)) AS sku,(select o.price from (profileshop.order_item o join profileshop.purchase_item_order_item pioi on((o.id = pioi.order_item_id))) where (pioi.purchase_item_id = p.id)) AS price,(select o.url from (shop.order_item o join profileshop.purchase_item_order_item pioi on((o.id = pioi.order_item_id))) where (pioi.purchase_item_id = p.id)) AS url,(select o.weight from (shop.order_item o join profileshop.purchase_item_order_item pioi on((o.id = pioi.order_item_id))) where (pioi.purchase_item_id = p.id)) AS weight,(select o.image from (shop.order_item o join profileshop.purchase_item_order_item pioi on((o.id = pioi.order_item_id))) where (pioi.purchase_item_id = p.id)) AS image,(select o.id from (shop.order_item o join profileshop.purchase_item_order_item pioi on((o.id = pioi.order_item_id))) where (pioi.purchase_item_id = p.id)) AS orderItemId,(select o.order_id from (shop.order_item o join profileshop.purchase_item_order_item pioi on((o.id = pioi.order_item_id))) where (pioi.purchase_item_id = p.id)) AS orderId,(select pp.merchant_id from profileshop.purchase pp where (pp.id = p.purchase_id)) AS merchantId from ((((((shop.purchase_item p join profileshop.purchase_item_order_item pioi on((p.id = pioi.purchase_item_id))) join profileshop.order_item o on((o.id = pioi.order_item_id))) left join purchase_shipment ps on((p.id = ps.purchase_item_id))) left join shipment_item si on((ps.shipment_item_id = si.id))) left join shipment s on(((s.id = si.id) and (s.shipment_type = 'PURCHASE')))) left join shipment_receipt sr on((si.id = sr.shipment_item_id))) where purchase_id = :po group by p.id having (p.quantity > preallocated)) AS x WHERE sku = :sku ", nativeQuery=true)
    ...
    @Query(value="SELECT si.id, si.sequence, si.quantity, si.description, si.shipment_id as shipmentId, si.product_id as productId, si.quantity - ifnull(sum(pc.quantity),0) as unpacked, ifnull(p.image,'') as image " +
        ",  (SELECT o.id from  profileshop.order_item o join profileshop.purchase_item_order_item pioi on o.id = pioi.order_item_id where pioi.purchase_item_id = ps.purchase_item_Id limit 1) AS orderItemId " +
        ",  (SELECT o.order_id from  profileshop.order_item o join profileshop.purchase_item_order_item pioi on o.id = pioi.order_item_id where pioi.purchase_item_id = ps.purchase_item_Id limit 1) AS orderId " +
        "FROM shipment_item si " +
        "LEFT JOIN packaging_content pc ON si.id = pc.shipment_item_id " +
        "LEFT JOIN profileshop.product p ON si.product_id = p.ref " +
    ...
    @Query(value="SELECT si.id as shipmentItemId, si.description, oi.image, si.quantity, s.id, s.shipment_type AS type, s.shipment_status AS status, s.last_modified_date AS date, s.tracking_num as trackingNum, s.shipment_method as carrier FROM shipment s " +
        ...
        "JOIN profileshop.purchase_item_order_item pioi ON pioi.purchase_item_id = ps.purchase_item_id " +
        "JOIN profileshop.order_item oi ON pioi.order_item_id = oi.id " +
        "JOIN profileshop.jhi_order o ON oi.order_id = o.id " +
        ...
        "JOIN profileshop.order_item oi ON oi.id = os.order_item_id " +
    ...
    @Query(value="select s.id AS id,s.created_date AS createdDate,s.shipment_method AS shipmentMethod,s.tracking_num AS trackingNum,s.pkg_count AS pkgCount,count(p.id) AS arrivedPkgs,s.shipment_status AS STATUS, " +
        ...
        "(select m2.name from profileshop.merchant m2 where s.merchant_id = m2.id) AS sender " +
        "from  shipment s " +
    ...
    @Query(value="select m.name as receiver, s.id AS id,s.created_date AS createdDate,s.shipment_method AS shipmentMethod,s.tracking_num AS trackingNum, s.shipment_status AS status from shipment s left join profileshop.merchant m on concat('m',m.id)  = s._to where s.shipment_type = :shipmentType AND s.shipment_status <> :shipmentStatus", nativeQuery = true)
    ...
    @Query(value="select  " +
        ...
        "from shipment_item si left join profileshop.product p on p.ref = si.product_id " +
        "left join purchase_shipment ps ON ps.shipment_item_id = si.id " +
        "left join profileshop.purchase_item pi2 on pi2.id = ps.purchase_item_id  " +
    ...
    @Query(value="select  " +
        ...
        "          p.image as image " +
        "from shipment_item si  " +
        "left join profileshop.product p on p.ref = si.product_id  " +
    ...
    @Query (value="select oi.id, oi.product_name as description, oi.image, o.reference,  pui.id as pid, pui.purchase_id as po, o.created_date as orderDate, o.invoice_date as invoiceDate, p.created_date as purchaseDate,  " +
            "pr.url, pr.sku, m2.name as merchant, p.merchant_id as merchantId, pui.product_id as productId, pui.price, " +
            "(select group_concat(concat(s2.id,':',s2.tracking_num,':',s2.shipment_method,':',s2.shipment_status,':',ifnull(m.name,''))) from purchase_shipment ps left join shipment_item si ON si.id = ps.shipment_item_id left JOIN shipment s2 on si.shipment_id  = s2.id and s2.shipment_type = 'TRANSIT' left join profileshop.merchant m on CONCAT('m', m.id) = s2.`_to`   where ps.purchase_item_id = pui.id group by pui.id ) as transitShipments,  " +
            "(select group_concat(concat(s2.id,':',s2.tracking_num,':',s2.shipment_method,':',s2.shipment_status,':',ifnull(m.name,'Badals'))) from purchase_shipment ps left join shipment_item si ON si.id = ps.shipment_item_id left JOIN shipment s2 on si.shipment_id  = s2.id and s2.shipment_type = 'PURCHASE'  left join profileshop.merchant m on CONCAT('m', m.id) = s2.`_to`   where ps.purchase_item_id = pui.id group by pui.id ) as purchaseShipments,  " +
            ...
            "oi.quantity " +
            "from profileshop.order_item oi left JOIN profileshop.jhi_order o ON oi.order_id = o.id    " +
            ...
            "left join profileshop.purchase p on p.id = pui.purchase_id " +
            "left join profileshop.merchant m2 on m2.id = p.merchant_id " +
            "left join profileshop.product pr on pr.`ref` = oi.product_id " +
            "where " +
